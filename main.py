...
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")
SLACK_WEBHOOK_URL = os.getenv("SLACK_WEBHOOK_URL")
HUGGINGFACE_MODEL_ID = os.getenv("HUGGINGFACE_MODEL_ID", "openai/gpt-oss-120b")
HUGGINGFACE_API_TOKEN = os.getenv("Hugging_Face_Api")
HUGGINGFACE_TIMEOUT = float(os.getenv("HUGGINGFACE_TIMEOUT", "120"))
...
def _extract_generated_text(result) -> str:
    """
    Hugging Face inference responses can be lists, dicts, or plain strings.
    Normalise them into a single text string.
    """
...
async def invoke_huggingface_inference(
    prompt: str,
    *,
    ...
    if not HUGGINGFACE_API_TOKEN:
        raise HTTPException(
            status_code=500,
            detail="Hugging Face API token is not configured. Set Hugging_Face_Api in the environment.",
        )
    ...
    try:
        async with httpx.AsyncClient(timeout=HUGGINGFACE_TIMEOUT) as client:
            response = await client.post(HUGGINGFACE_API_URL, headers=headers, json=payload)
            response.raise_for_status()
            result = response.json()
            generated = _extract_generated_text(result).strip()
            if not generated:
                raise HTTPException(
                    status_code=502,
                    detail="Empty response from Hugging Face Inference API.",
                )
            return generated
        ...
        raise HTTPException(
            ...
            detail=f"Hugging Face API error ({status}): {detail}",
        ) from exc
    except Exception as exc:
        raise HTTPException(
            status_code=502,
            detail=f"Failed to call Hugging Face API: {str(exc)}",
        ) from exc
...
async def generate_summary(transcript: str, speakers_data: List[dict]):
    ...
    f"Transcript:\n{formatted_transcript}"
        ...
        return await invoke_huggingface_inference(
            summary_prompt,
            temperature=0.1,
            top_p=0.9,
            top_k=40,
...
Provide a clear, specific answer based only on the information in the transcript. If the answer is not in the transcript, say so."""
        ...
        answer = await invoke_huggingface_inference(
            question_prompt,
            temperature=0.3,
            top_p=0.9,
            top_k=40,
